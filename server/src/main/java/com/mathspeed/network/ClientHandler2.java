...
// *
// * - Gọi trực tiếp các API của ClientRegistry, Matchmaker, ChallengeManager và PlayerDAO theo chữ ký bạn cung cấp.
// * - Gửi/nhận messages theo MessageType enum (text form): "TYPE" hoặc "TYPE|payload".
// * - Giữ writer persist để push message từ server tới client hiệu quả.
// * - Đảm bảo đóng socket/streams trong finally, unregister và logout ở cleanup.
// *
// * Sửa đổi chính:
// * - Thêm field userId (UUID string) và set khi login thành công từ Player trả về bởi PlayerDAO.login(...)
// * - getUserId() trả về userId (thay vì dùng undefined id)
// * - Khi mapping/ sử dụng game/session luôn ưu tiên dùng userId nếu cần (GameDAO, GameSession, v.v.)
// * - Đảm bảo registerClient vẫn dùng username nếu registry giao diện yêu cầu username; nếu registry muốn id,
// *   điều chỉnh call tương ứng trong phần tạo session/manager (thường registry cần username).
// */
